...
class BatchEntropyLossLayer(caffe.Layer):
   ...
   def forward(self, bottom, top):
      ...
      self.p_dist = np.multiply(np.exp(-self.beta_ * bottom[0].data), 1.0/np.array([np.sum(np.exp(-self.beta_ * bottom[0].data), axis=0),]*bottom[0].data.shape[0]))
      # Now we compute entropy over p distribution
