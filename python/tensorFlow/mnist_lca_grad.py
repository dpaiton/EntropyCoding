...
n_    = 784         # Num_pixels
m_    = 400         # Num_elements
lamb_ = 0.70        # Threshold potential (sparseness penalty scaling)
...
num_trials_ = 3000  # Number of batches to learn weights
display_ = 50       # How often to display status updates
thresh_ = 'soft'    # Thresholding type for LCA -> can be 'hard' or 'soft'

# Gradient Descent Parameters
lr_   = 0.10        # Learning rate for weight updates
...
def T(u, lamb, thresh_type='soft'):
  ...
  if thresh_type == 'soft':
    return tf.select(tf.greater(u, lamb), u-lamb, tf.zeros(shape=tf.shape(u), dtype=tf.float32))
  else:
    return tf.select(tf.greater(u, lamb), u, tf.zeros(shape=tf.shape(u), dtype=tf.float32))
...
s = tf.placeholder(tf.float32, shape=[n_, batch_]) # Placeholder for data
...
du = (1-eta) * u + eta * tf.sub(tf.matmul(tf.transpose(phi), s),
  tf.matmul(G(phi), T(u, lamb_, thresh_type=thresh_)))
#du = tf.sub(tf.matmul(tf.transpose(phi), s),
#  tf.matmul(G(phi), T(u, lamb_, thresh_type=thresh_)))
...
train_weights = tf.train.GradientDescentOptimizer(lr_, name="grad_optimizer").minimize(unsupervised_loss,
  var_list=[phi], name="grad_minimizer")
...
for trial in range(num_trials_):
  ...
  train_weights.run({s:norm_input})

  ## Renormalize weights after update
  normalize_phi.run()

  if trial % display_ == 0 and display_ > 0:
